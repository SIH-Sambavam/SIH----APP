...
class AppCtrl extends ChangeNotifier {
  ...
  // Camera and screen share states
  ...
  void connect() async {
    ...
    try {
      // Generate room and participant names for BlueGuard marine assistant
      final roomName =
          'blueguard-marine-${(1000 + DateTime.now().millisecondsSinceEpoch % 9000)}';
      final participantName =
          'marine-user-${(1000 + DateTime.now().millisecondsSinceEpoch % 9000)}';
      ...
      _logger.info("Fetched Connection Details: connecting to room...");
  ...
  void _startAgentConnectionTimer() {
    ...
    _agentConnectionTimer = Timer.periodic(const Duration(seconds: 1), (timer) {
      ...
      // If 20 seconds have elapsed and no agent found, disconnect
      if (timer.tick >= 20) {
        _logger.warning(
